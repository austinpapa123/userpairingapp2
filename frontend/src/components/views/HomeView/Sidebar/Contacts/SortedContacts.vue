...
function deleteContact(delete_contact_id: number) {
  console.log("deleting the contact");
  store.deleteContact(delete_contact_id);
}
  ...
  <div v-for="(group, groupIndex) in props.contactGroups" :key="groupIndex">
    ...
    <div v-for="(contact, index) in group.contacts" :key="index">
      <div class="w-full p-5 flex justify-between items-center">
        ...
        <Vue3StatusIndicator v-if="contact.status === 'online'" type="success" class="w-3 h-3"/>
        <Vue3StatusIndicator v-else type="warning" class="w-3 h-3"/>
        ...
        <div class="relative">
          ...
          >
            ...
            <DropdownLink color="danger" @click="deleteContact(contact.id)">
